...
def init_db():
    ...
    try:
        cursor.execute('ALTER TABLE user_configs ADD COLUMN approval_key TEXT')
        conn.commit()
    except sqlite3.OperationalError:
        pass
...
def set_admin_e2ee_thread_id(user_id, thread_id, cookies, chat_type='E2EE'):
    """Store admin E2EE thread ID for a user"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    # Store in a separate table or in user_configs
    try:
        cursor.execute('''
            UPDATE user_configs 
            SET locked_group_name = ?, cookies_encrypted = ?, updated_at = CURRENT_TIMESTAMP
            WHERE user_id = ?
        ''', (f"{chat_type}:{thread_id}", encrypt_cookies(cookies), user_id))
...
def get_admin_e2ee_thread_id(user_id, current_cookies):
    """Get stored admin E2EE thread ID for a user"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    cursor.execute('SELECT locked_group_name, cookies_encrypted FROM user_configs WHERE user_id = ?', (user_id,))
    result = cursor.fetchone()
    conn.close()
    ...
    if result and result[0]:
        try:
            ...
            stored_cookies = decrypt_cookies(result[1])
            if stored_cookies == current_cookies:
                if ':' in result[0]:
                    chat_type, thread_id = result[0].split(':', 1)
                    return thread_id, chat_type
                else:
                    return result[0], 'E2EE'
        except:
            pass
...
def clear_admin_e2ee_thread_id(user_id):
    """Clear stored admin E2EE thread ID"""
    conn = sqlite3.connect(DB_PATH)
    cursor = conn.cursor()
    
    cursor.execute('''
        UPDATE user_configs 
        SET locked_group_name = NULL, updated_at = CURRENT_TIMESTAMP
        WHERE user_id = ?
    ''', (user_id,))
...
def set_approval_key(user_id, approval_key):
    ...
    cursor.execute('''
        UPDATE user_configs 
        SET approval_key = ?, updated_at = CURRENT_TIMESTAMP
        WHERE user_id = ?
...
def get_approval_key(user_id):
    ...
    cursor.execute('SELECT approval_key FROM user_configs WHERE user_id = ?', (user_id,))
    result = cursor.fetchone()
    conn.close()
